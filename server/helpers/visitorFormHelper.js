  ...
  req.on("end", () => {
    try {
      ...
      if (action === "add") {
        ...
        const sql =
          "INSERT INTO visitors (first_name, Minit_name, last_name, username, password, email, phone_number, date_of_birth, gender, street_address, city, state, zipcode, country, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        const values = [
          first_name,
          Minit_name,
          last_name,
        ...
        const sql =
          "UPDATE visitors SET first_name = ?, Minit_name = ?, last_name = ?, username = ?, password = ?, email = ?, phone_number = ?, date_of_birth = ?, gender = ?, street_address = ?, city = ?, state = ?, zipcode = ?, country = ?, role = ? WHERE visitor_id = ?";
        const values = [
          first_name,
          Minit_name,
          last_name,
        ...
        if (!visitor_id) {
          ...
          res.end(
            JSON.stringify({
              success: false,
              message: "Visitor ID is required for deletion",
            })
          );
          return;
        }
