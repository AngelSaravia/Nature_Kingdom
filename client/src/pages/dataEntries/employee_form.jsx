...
const EmployeeForm = ({ employeeData, source, onClose}) => {
    ...
    useEffect(() => {
        // Prioritize employeeData if provided
        if (employeeData) {
            console.log("Using employeeData from props:", employeeData); // Debugging log
            setFormData(employeeData);
            setIsEditMode(true); // Set edit mode to true if data is passed via props
        } else {
            // Fallback to location.state or sessionStorage
            const tupleData = location.state?.tuple || JSON.parse(sessionStorage.getItem('employeeEditData') || null);
    
            if (tupleData) {
                console.log("Using tupleData from location or sessionStorage:", tupleData); // Debugging log
                setFormData({
                ...
                setIsEditMode(true); // Set edit mode to true if data is passed via location or sessionStorage
                sessionStorage.removeItem('employeeEditData'); // Clear sessionStorage after use
            } else {
                console.log("No data found - creating new form"); // Debugging log
                setFormData({
                    Employee_id: "",
                    first_name: "",
                    Minit_name: "",
    ...
    const handleSubmit = async (action) => {
        ...
        try {
            ...
            if (result.success) {
                if (onClose) {
                    onClose(); // Close the popup if onClose is provided
                }
                if (action === "delete") {
    ...
    return (
        <div className={source === "manager_query_report" ? styles.popupContainer : styles.formContainer}>
            <div className={source === "manager_query_report" ? styles.popup : ""}>
                {/* Conditionally render the link to the Employee Query Report */}
                {source !== "manager_query_report" && (
                    <div className={styles.queryReportLink}>
                        <Link to="/query_report/employees" className={styles.queryReportButton}>
            ...
            <form className={styles.form} onSubmit={(e) => e.preventDefault()}>
                ...
                <div className={styles.formRow}>
                    ...
                    <InputFields label="DEPARTMENT ID *" name="department_id" type="text" value={formData.department_id} onChange={handleChange} pattern="[0-9]+" onInput={handleNumericInput} autoComplete="off" readOnly={source === "manager_query_report"} />
                </div>
                ...
                <div className={styles.formRow}>
                    ...
                    <InputFields label="MANAGER ID" name="Manager_id" type="text" value={formData.Manager_id} onChange={handleChange} pattern="[0-9]+" onInput={handleNumericInput} required={false} autoComplete="off" readOnly={source === "manager_query_report"} />
                </div>
