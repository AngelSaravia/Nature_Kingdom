...
const MedicalRecordsReport = () => {
    ...
    useEffect(() => {
        const fetchFilteredSummary = async () => {
            try {
                ...
                setEmployeesOptions(data.employees.map(emp => ({
                    value: emp.id,
                    label: emp.name
                })));
                setRecordTypesOptions(data.recordTypes.map(type => ({
                    value: type.id,
                    label: type.name
                })));
    
                // Set species and enclosures options if available
                setSpeciesOptions(data.species.map(species => ({
                    value: species,
                    label: species
                })));
                setEnclosuresOptions(data.enclosures.map(enclosure => ({
                    value: enclosure.id,
    ...
    const handleAnimalSpeciesChange = selectedOptions => {
        setFilters(prevState => ({ ...prevState, animalSpecies: selectedOptions }));
    };
                        ...
                        <div className="summary-section">
                            ...
                            <div className="summary-table">
                                <div className="summary-row">
                                    ...
                                    <div className="summary-value">{summaryData.totalMedicalRecords}</div>
                                </div>
                                <div className="summary-row">
                                    <div className="summary-label">Total Unique Animals With Records</div> 
                                    <div className="summary-value">{summaryData.uniqueAnimalsWithRecords}</div>
                                </div>
                                <div className="summary-row">
                                    <div className="summary-label">Most Common Record Type</div>
                                    <div className="summary-value">{summaryData.mostCommonRecordType}</div>
                                </div>
                ...
                {/* Detailed Records Section */}
                    <div className="detailed-records-section">
                        ...
                        {detailedRecords.length === 0 ? (
                            <p className="no-records-message">No medical records found for the selected filters.</p>
                        ) : (
