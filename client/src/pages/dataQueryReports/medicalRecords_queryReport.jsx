  ...
  const fetchDropdownData = async () => {
    try {
      const [employeeEmails, enclosureNames, species] = await Promise.all([
        fetch(
          `${API_BASE_URL}/medical_records/distinct_values?field=email&table=employees`
        ).then((res) => res.json()),
        fetch(
          `${API_BASE_URL}/medical_records/distinct_values?field=name&table=enclosures`
        ).then((res) => res.json()),
        fetch(
          `${API_BASE_URL}/medical_records/distinct_values?field=species&table=animals`
        ).then((res) => res.json()),
      ]);
  ...
  const fetchReport = async (applyFilters = true) => {
    try {
      ...
      const queryParams = {
        ...
        additional_joins: [
          ...
          },
          {
            table: "enclosures",
            join_condition:
              "medical_records.enclosure_id = enclosures.enclosure_id",
          },
        ],
        computed_fields: `
                    medical_records.*,
      ...
      const response = await fetch(
        `${API_BASE_URL}/entry_form/medicalRecords`,
        {
          method: "POST",
    ...
    <div className="medicalRecords-query-report">
      ...
      <div className="report-table-container">
        ...
        <div className="edit-medicalRecords-button-container">
          <Link to="/medical_form" className="edit-medicalRecords-button">
            Add Medical Record
          </Link>
        </div>
      </div>
