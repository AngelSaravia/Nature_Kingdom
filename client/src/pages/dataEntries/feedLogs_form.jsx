...
const FeedLogsForm = () => {
  ...
  // Move formatDateTime function to component scope so it can be used by both useEffect and handleSubmit
  const formatDateTime = (dateTime) => {
    if (!dateTime) return "";
    return new Date(dateTime).toISOString().slice(0, 16); // Format as 'YYYY-MM-DD HH:mm'
  };
  ...
  const handleSubmit = async (action) => {
    ...
    if (action === "delete" && !formData.schedule_id) {
      setSubmissionStatus({
        success: false,
        message: "Feed schedule ID required for deletion.",
      });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        if (action === "delete") {
          ...
          if (freshData.success) setfeedLogs(freshData.data);
        }
        if (action !== "delete") {
          setFormData({
            schedule_id: "",
