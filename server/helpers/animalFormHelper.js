  ...
  req.on("end", () => {
    try {
      ...
      if (action === "add") {
        ...
        const sql =
          "INSERT INTO animals (animal_name, date_of_birth, enclosure_id, species, animal_type, health_status) VALUES (?, ?, ?, ?, ?, ?)";
        const values = [
          animal_name,
          date_of_birth,
          enclosure_id,
        ...
        const sql =
          "UPDATE animals SET animal_name = ?, date_of_birth = ?, enclosure_id = ?, species = ?, animal_type = ?, health_status = ? WHERE animal_id = ?";
        const values = [
          animal_name,
          date_of_birth,
          enclosure_id,
        ...
        if (!animal_id) {
          ...
          res.end(
            JSON.stringify({
              success: false,
              message: "Animal ID is required for deletion",
            })
          );
          return;
        }
