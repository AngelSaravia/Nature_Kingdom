...
function handleEmployeeForm(req, res) {
    ...
    req.on("end", () => {
      try {
        ...
        const { action, Employee_id, first_name, last_name, date_of_birth, salary, user_name, password, department_id, gender, email, phone} = formData;
        ...
        if (action === "add") {
          if (!first_name || !last_name || !date_of_birth || !salary || !user_name || !password || !department_id || !gender || !email || !phone) {
        ...
        const sql = "UPDATE employees SET first_name = ?, last_name = ?, date_of_birth = ?, salary = ?, user_name = ?, password = ?, department_id = ?, gender = ?, email = ?, phone = ?, Manager_id = ?, Minit_name = ?, street_address = ?, city = ?, state = ?, zip_code = ?, country = ? WHERE Employee_id = ?";
        const values = [first_name, last_name, date_of_birth, salary, user_name, password, department_id, gender, email, phone, Manager_id, Minit_name, street_address, city, state, zip_code, country, Employee_id];
